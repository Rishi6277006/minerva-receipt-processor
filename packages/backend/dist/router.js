...
exports.appRouter = t.router({
    ...
    receipt: t.router({
        parseAndAdd: t.procedure
            .input(zod_1.z.object({
            receiptText: zod_1.z.string()
        }))
            .mutation(async ({ ctx, input }) => {
            try {
                const parsedReceipt = await (0, openaiService_1.parseReceiptWithOpenAI)(input.receiptText);
        ...
        uploadImage: t.procedure
            .input(zod_1.z.object({
            imageData: zod_1.z.string().refine((data) => {
            ...
            try {
                ...
                // Process the image
                const result = await imageService_1.ImageService.processReceiptImage(imageBuffer, input.filename);
                return {
                    success: true,
                    entry: result.entry,
    ...
    bank: t.router({
        ...
        uploadStatement: t.procedure
            ...
            try {
                ...
                const dateIndex = headers.findIndex(h => h.includes('date') || h.includes('transaction_date') || h.includes('trans_date'));
                const descriptionIndex = headers.findIndex(h => h.includes('description') || h.includes('desc') || h.includes('memo') || h.includes('note') || h.includes('details'));
                const depositsIndex = headers.findIndex(h => h.includes('deposits') || h.includes('deposit') || h.includes('credit'));
                const withdrawalsIndex = headers.findIndex(h => h.includes('withdrawls') || h.includes('withdrawals') || h.includes('withdrawal') || h.includes('debit'));
                const amountIndex = headers.findIndex(h => h.includes('amount') || h.includes('amt') || h.includes('sum') || h.includes('total'));
                const typeIndex = headers.findIndex(h => h.includes('type') || h.includes('transaction_type') || h.includes('category'));
    ...
    comparison: t.router({
        compare: t.procedure.query(async ({ ctx }) => {
            ...
            ledgerEntries.forEach((ledger) => {
                const matchingBankTxIndex = bankOnly.findIndex((bank) => Math.abs(ledger.amount - bank.amount) < 0.01 && // Amount tolerance
                ...
                if (matchingBankTxIndex !== -1) {
                    ...
                    const ledgerIndex = ledgerOnly.findIndex((l) => l.id === ledger.id);
                    if (ledgerIndex !== -1)
                        ledgerOnly.splice(ledgerIndex, 1);
